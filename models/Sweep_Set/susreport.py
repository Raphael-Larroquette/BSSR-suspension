...
def write_report(sweeps: list[Sweep], summary: pd.DataFrame,
                 side: str, out: Path, plots: list[Path],
                 joints: list[JointResult] | None = None) -> Path:
    joints = joints or []
    lines: list[str] = []
    ...
    if joints:
        ...
        lines.append(
            ...
            "per sweep. Install offset is the angle between the housing and "
            "bore directions at the neutral pose: non-zero means the bearing "
            "is fitted deliberately off centre and starts already eating part "
            "of its cone. The best-available column is what the joint would "
            "need if its bore axis were chosen to minimise the worst case.\n"
        )
        lines.append("| joint | part | required (deg) | at | install offset (deg) "
        ...
        banded = [j for j in joints if j.locked_required is not None]
        if banded:
            lines.append("### Members whose spin is undetermined\n")
            lines.append(
                "These housings ride a two-point member, whose roll about its "
                "own axis no kinematic model can determine. The lower bound "
                "assumes the member turns freely to the best position at every "
                "instant; the locked value assumes it never turns, so one "
                "clocking chosen at assembly serves the whole sweep. Size the "
                "bearing against the locked value unless you know the member "
                "runs free.\n"
            )
            lines.append("| joint | free-spin lower bound (deg) | locked (deg) "
                         "| locked clocking (deg) |")
            lines.append("| --- | --- | --- | --- |")
            for j in banded:
                low = "-" if j.required is None else f"{j.required:.2f}"
                lines.append(f"| {j.label} | {low} | {j.locked_required:.2f} | "
                             f"{j.locked_clocking_deg:.1f} |")
            lines.append("")

        lines.append("### How each joint was resolved\n")
        for j in joints:
            note = f" ({j.note})" if j.note else ""
            lines.append(f"- **{j.label}** -- {j.description}{note}")
        lines.append("")
...
def main() -> None:
    ...
    joints = analyse_joints(sweeps, args.side)
    if joints:
        pd.DataFrame(
            [
                {
    ...
    report = write_report(sweeps, summary, args.side, out, plots, joints)
